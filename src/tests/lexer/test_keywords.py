...
"""
        ...
        assert tokens[0] == IdentifierValueToken(
            value="foreachtest", length=11, startPosition=Position(line=1, column=1)
        )
        assert tokens[1] == IdentifierValueToken(
            value="foreach_test", length=12, startPosition=Position(line=2, column=1)
        )
    ...
    def testAllTokens(self):
        ...
        assert tokens[45] == IdentifierValueToken(
            startPosition=Position(line=46, column=1), value="identifier", length=10
        )
